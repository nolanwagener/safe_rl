...
if __name__ == '__main__':
    ...
    # Environment
    parser.add_argument('--env', type=str, default='extra_envs:Point-v0')
    ...
    # Intervention
    parser.add_argument('--intv_config', type=str, default='intervener/empty.yaml',
                        help="Path to intervener config file")
    parser.add_argument('--no_intv', action='store_true')
    ...
    with open(args.intv_config) as f:
        intervener_kwargs = yaml.load(f, Loader=yaml.FullLoader)
        if intervener_kwargs is None:
            intervener_kwargs = dict()

    def env_fn():
        env = gym.make(args.env)
        IntervenerCls = Intervener if args.no_intv else env.INTERVENER_CLS
        intervener = IntervenerCls(**intervener_kwargs)
    ...
    cppo(env_fn, actor_critic=core.MLPActorCritic,
         ...
         dont_normalize_adv=args.dont_normalize_adv, logger_kwargs=logger_kwargs,
         cost_lim=args.cost_lim, penalty=args.penalty,
         optimize_penalty=args.optimize_penalty, penalty_lr=args.penalty_lr,
         ignore_unsafe_cost=args.ignore_unsafe_cost, ent_bonus=args.ent_bonus,
